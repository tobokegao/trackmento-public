...
「柱・帯」（塊を枠の辺にぴったり付ける）と「マスごと」（1 列の並びで曲名をマスの横に並べる）を足し、
下に置く曲名リストも列を割るようにした回。<b>2950 件のうち 543 件でマスが大きくなり、小さくなったのは 70 件</b>
（すべて比率なしの 1〜3 列と 1xN の 16:9 で、曲名リストにまともな幅を与えるぶんの代償）。
要確認は 192 → 57 件、「タイトルが目立たない」は 20 → 0 件。
