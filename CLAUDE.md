...
## 直近の数字（2026-09-14 13:38 の点検）

判定「正常」。以下は次回の比較用。

- 帯域: 直近 **0.26 GB/2h**（08:26 は 0.22、対策前 1.24）。今月累計は未取得（Render の Billing でしか見られない。
  09-14 08:26 の時点で 111.06 GB / 25 GB 含む ＝ 超過 86.06GB ＝ $12.91）
- メモリ最大 213MB、CPU 最大 0.024、**5xx 合計 0**、エラー行 0、`[loop] lag` 0 行
- デプロイ 4 回（12:48 / 13:17 / 13:27。マスの表示・R2 の取り直し・曲名リストの流し込み）
- 共有数 **1753 件/日**（09-14 00:10 は 5803 件）。バズが落ち着いてきている
- UA: 人 98%
- **`/s/*` の最大応答 0.7 秒**（08:26 は 6.4 秒）。前回の気がかりは解消した

### 次回に見るもの

- **`/image-proxy` が増えている**。527 件 → **1402 件**、最大 1.0 秒 → **3.8 秒**、ピーク 40 → **365 件/分**。
  これは `direct=1` のフォールバック（R2 から読めなかったぶんをサーバーが本体で返す）が実際に効いている証拠で、
  ジャケットの穴が埋まる代わりに転送量と処理が増える。**R2 側の失敗そのものは残っている**
  - 本筋の解決は、R2 の公開 URL を `r2.dev`（Cloudflare が開発用としていてレート制限がある）から
    **カスタムドメイン**に変えること。バケットにカスタムドメインを割り当てれば制限が外れる。
    Cloudflare のダッシュボード操作が要る（API トークンでは変えられない）
  - 高止まりするか、帯域が目に見えて増えるようなら手を付ける
