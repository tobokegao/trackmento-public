...
- **つまみ（余白・マスの間隔・色の指定）は掴まないと動かない**（`thumbOnly`）。`input[type="range"]` は
  ...
  - **既定の「押した位置へ値が飛ぶ」は常に止め、ドラッグは自前で動かす**（2026-09-15）。
    「つまみの上なら既定に任せる」形にしていたときは次の 2 つが残っていた:
    つまみには幅があるので**押しただけでも値がその位置へずれる**（連打すると動く）／
    指の太さぶんの遊び（±8px）を足していたので**つまみの真横の溝でも動く**。
    今は `pointerdown` を必ず `preventDefault` し、つまみの上のときだけ `setPointerCapture` して
    `pointermove` で値を書く。**キーボード（矢印キー）は触っていないので今までどおり効く**
  ...
  - 確かめ方: `node promo/test_slider.mjs`（マウス）、`node promo/test_slider.mjs touch`（指）。
    つまみ連打・真横連打・真横ドラッグ・見出し長押し・真横をタップして滑らせる、のどれでも
    値が変わらず、つまみのドラッグだけ変わる
...
- **つまみの四角と握りの縞は背景（`background-image` の重ね）で描く**（スライダーもスクロールバーも同じ規則）。
  **面取りを `box-shadow: inset` で描かない**のは、内側の影が背景より上に乗るため**角の 1px だけ塗り分けられない**から。
  ...
  - **タイトルバーの縞も同じ規則**（2026-09-15）。先頭（いちばん上）の 1 本だけ純白、以降は `--color-hl-dim`。
    全部を純白にすると光の当たっている面が分からず、のっぺり見える（`.pane-title` の背景）
