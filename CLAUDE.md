...
**機械で言える分は `scripts/check.py` が門になっている**（2026-09-27）。コミット前に変えたファイルに応じて回り、push 前に R2 の上げ忘れを見て、push 後に CI（`check.yml`）がもう一度見る。落ちたら表の「回すもの」を回す。**作業の終わりの条件は「`check.py --all` が通る」**。`compare_render.py` と `compare_trim.py` はサーバーとブラウザが要るので門では回らない（該当ファイルを変えると回すよう表示される）。
