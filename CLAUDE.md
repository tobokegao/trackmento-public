...
## 直近の数字（2026-09-14 20:37 の点検）

判定「正常」。**利用が目に見えて増えた回**。以下は次回の比較用（括弧内は 18:29 の値）。

- 帯域: 直近 **0.66 GB/2h**（0.42 → 0.26 → 0.22、対策前 1.24）。最大 0.33 GB/時。
  このペースが続くと月 237GB ＝ 超過分で約 $32/月。今月累計は 09-14 08:26 時点で 111.06 GB / 25 GB 含む ＝ $12.91
- 要求: `/grids/*` **6438 件**（3392）、`/image-proxy` **2980 件**（1842）、`/search` **1252 件**、
  `/health` 1271 件、`/` **1140 件**（555）、`/s/*` **880 件**（539）、`/from-url` **371 件**。
  **どの経路もおおむね倍**。人が増えたぶんで、ボットの増加ではない（人 97%）
- 共有数 **5948 件/日**、メモリ最大 200MB、`[health] rss` 最大 279MB、CPU 最大 0.062、エラー行 0、`[loop] lag` 0 行
- **5xx 6 件**（前回 1）。うち実害があるのは共有の 2 件:
  - `503 /share/upload 共有が混み合っています`（`_UPLOAD_SEM` = 3 に当たった）
  - `503 /share 共有の生成が混み合っています`（`MAX_RENDER_QUEUE` = 2 に当たった）
  - 残りは配信元都合: `502 /image-proxy 画像サーバーが 404`（3 件）、`502 /from-url Bandcamp が 403`（1 件）

### 共有の 503 をいつ直すか

**この 2 つの上限は無料プラン（0.1 vCPU・512MB）の頃の値**で、今の Standard（1 vCPU・2048MB）には
きつすぎる。CPU 最大 0.062、メモリ 200MB / 2048MB なので、上げても余力は十分ある。

- **`_UPLOAD_SEM = 3`（`/share/upload`）** … 受け取って保存するだけで CPU はほぼ使わない。
  効いているのは「本文（数 MB）を抱えたまま並ぶとメモリが膨らむ」ことだけ。
  1 件あたり最大 10MB としても 8 並列で 80MB。**2 時間で 5 件を超えたら 8 に上げる**
- **`MAX_RENDER_QUEUE = 2`（`/share`）** … こちらはサーバー描画で、1 件 40〜80 秒かかる重い処理。
  数を増やすとイベントループが詰まってヘルスチェックに落ちる危険があるので、**安易に上げない**。
  そもそも通るのは Canvas が使えない端末だけなので、増えているなら**どの端末で落ちているか**を先に調べる
- **どちらも「2 時間で 1〜2 件」なら放置でよい**。利用者は Retry-After のとおり待てば通る

### 次回に見るもの

- **共有の 503 の件数**。2 時間で 5 件を超えたら `_UPLOAD_SEM` を上げる（上の節）
- **`/s/*` の最大応答が 12.4 秒**（18:29 は 12.8 秒、13:38 は 0.7 秒）。**2 回続いた**ので
  「たまたま 1 件」では説明しにくい。880 件中の最大値で 5xx は 0。
  3 回続くなら R2 の接続待ち（`storage.py` の `connect_timeout` 3 秒）と、共有ページが読む
  オブジェクトの数を見直す
- **帯域の伸び**。利用が倍になっても 0.42 → 0.66 GB/2h で済んでいる（R2 逃がしが効いている）。
  件数の伸びに対して帯域が比例以上に増えたら、逃がせていない経路がある
