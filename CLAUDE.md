...
- **右に置く曲名リストの「1 行型」**（`render.py` の `_inline_plan` / `INLINE_GAIN` と frontend の `inlinePlan`、2026-09-18）。
  ...
  - 行数は「全曲 1 行」から始め、並ばない曲（曲名＋1 字ぶん＋アーティスト名が列に入らない）はアーティスト名を
    次の行の右端に落として行を増やし、字を決め直す。**曲名かアーティスト名が単独で 1 行に入らない曲があれば使わない**（折る組み方は 2 行型に任せる）
  ...
  - 実測: 3x3・16:9・9 曲で字 28 → 50px。compare_layout に `inline` の項目を足した（ランダムの 200 通りには
    1 行型の並びがほぼ入らないので、1〜6 マス四方 × 16:9 / free を別に突き合わせて 0 件）
...
- **検索の 1 ソースあたりの上限は 12 秒**（`main.py` の `SOURCE_TIMEOUT`、2026-09-16 に 20 から下げた）。
  ...
  - 点検の要約に**検索失敗の内訳**（ソースと理由）が出る（`render_check.py` の `search_fail_by`）
