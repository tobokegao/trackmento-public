...
## 直近の数字（2026-09-15 09:49 の点検）

判定「正常」。**R2 の CORS を直した効果が出た回**（09-15 04:5x に設定）。

- **`/image-proxy` の最大応答が 12.6 秒 → 0.9 秒**、**`502 … 429`（配信元のレート制限）が 3 件 → 0 件**。
  これが CORS 修正の直接の証拠。ブラウザが R2 から読めるようになり、**`direct=1` での取り直し
  （＝ Render を通して本体を返す経路）が無くなった**
- 帯域は**件数あたり**で比べる（時間帯で総量が変わるため）: **1 要求あたり 60 KB → 40 KB**
  （09-14 23:30 → 09-15 09:49）。朝でまだキャッシュが一巡していないので、ここからさらに下がるはず
- 共有の 5xx **0 件**。`/share/upload` の 503 も無し（前回 2 件）
- `[health] rss` 最大 **229MB**（05:06 は 328MB、23:30 は 270MB）。増え続けてはいない
- メモリ最大 209MB、CPU 最大 0.032、エラー行 0、`[loop] lag` 0 行
- 要求: `/grids/*` 3494、`/health` 1664、`/image-proxy` 1364、`/s/*` 705、`/` 600、`/search` 414。
  朝なので夜の 1/3 ほど
- 本日の共有数 **16463 件**（08:27 時点）
- **`/fonts/Silkscreen-Bold.woff2` が 253 件**サーバーから出ている。ピクセルフォントだけは R2 に載せていない
  （分割していないため）。1 件 10〜20KB で、まとめて数 MB。気になるなら R2 へ移せる

### 次回に見るもの

- **帯域の件数あたり**。09-14 23:30 の 60 KB/要求 が基準。キャッシュが一巡した 09-15 夜の値で最終確認
- `/share/upload` の 503（2 時間で 5 件を超えたら `_UPLOAD_SEM` を 3 → 8）
