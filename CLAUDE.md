...
## 直近の数字（2026-09-14 18:29 の点検）

判定「正常」。以下は次回の比較用。

- 帯域: 直近 **0.42 GB/2h**（13:38 は 0.26、08:26 は 0.22、対策前 1.24）。増えているが、
  **`/grids/*` 2320 → 3392 件、`/` 363 → 555 件と全体が伸びているので利用が増えたぶん**。
  今月累計は未取得（Render の Billing でしか見られない。09-14 08:26 時点で 111.06 GB / 25 GB 含む ＝ $12.91）
- メモリ最大 199MB、CPU 最大 0.035、エラー行 0、`[loop] lag` 0 行
- 要求: `/grids/*` 3392 件、`/image-proxy` 1842 件、`/health` 1611 件、`/` 555 件、`/s/*` 539 件
- UA: 人 97%。`/s/*` だけ人以外 41%（うちプレビュー 141 件）
- **5xx 1 件**: `502 /image-proxy 画像サーバーが 404 を返しました`。配信元が消した画像なので実害なし
- **R2 をカスタムドメインにした効果は続いている**。`/image-proxy` のピークは 105 件/分で、
  移行前の 365 件/分（`direct=1` のフォールバックが大量発生していた状態）には戻っていない

### 次回に見るもの

- **`/s/*` の最大応答が 12.8 秒**（13:38 は 0.7 秒、08:26 は 6.4 秒）。跳ね方が大きい。
  539 件中の最大値で 5xx は 0 なので、特定の 1 件が遅かっただけの可能性が高いが、
  続くようなら R2 の接続待ち（`storage.py` の `connect_timeout` 3 秒）をもう一度見る
