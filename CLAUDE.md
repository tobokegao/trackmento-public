...
## 直近の数字（2026-09-14 08:26 の点検）

判定「正常」。以下は次回の比較用。

- 帯域: 今月累計 **111.06 GB / 25 GB 含む** → 超過 86.06GB ＝ **$12.91**（前回 09-14 00:10 は 109 GB＝$12.75）。
  直近は **0.22 GB/2h**（前回 0.42、対策前 1.24）。前回の点検から 8.3 時間で 2.06GB 増えており平均 0.50 GB/2h なので、
  imgcache の索引を入れた 02:39 以降で下がっている
- メモリ最大 195MB、CPU 最大 0.023、**5xx 合計 0**、エラー行 0、`[loop] lag` 0 行
- 要求の内訳: `/grids/*` 1675 件、`/health` 1652 件、`/image-proxy` 527 件、`/s/*` 503 件、`/` 325 件、`/search` 280 件
- UA: 人 95%。`/s/*` だけ人以外 43%（うちプレビュー 137 件）で、`/s/*` の robots.txt を触らないと決めた根拠のまま
- **imgcache の索引は本番で動いている**。デプロイ（02:39:4x）の数秒後に
  `[storage] imgcache の索引: 5864 件（デプロイ後の取り直しを防ぐ）`
- R2: 今回は未取得。前回（09-14 00:10）はストレージ 12.96GB（30 日平均 7.9GB で無料枠内）、
  Class A 操作 18.5k/24h（30 日換算 555k ＝ 無料枠 100 万の 56%）。**09-11 の PNG 5.3GB が 09-18 に期限切れで消える**

### 次回に見るもの

- **`/s/*` の最大応答が 6.4 秒**。14.3 秒 → 1.0 秒に直した経路が戻っている。503 件中の最大値なので
  外れ値の可能性もあり、5xx は 0。続くようなら R2 の接続待ちをもう一度見る
