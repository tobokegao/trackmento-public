...
- **シート・モーダルを開いている間は body を `position: fixed` で固定する**（`lockScroll` / `unlockScroll`、
  2026-09-16）。`overflow: hidden` だけでは iOS Safari と一部の Android で指のスクロールが後ろの画面に
  抜ける（利用者の報告）。今の位置を `top` に入れて止め、閉じるときに `scrollTo` で戻す。
  中身の端まで行っても後ろに伝えないのは `overscroll-behavior: contain`（`.sheet-body` など）。
  **開閉の場所は 6＋6 か所あり、`sheet-open` の class を直接触らずこの 2 つを通す**
- **グリッド窓のボタンはスマホで 2×2**（曲を探す・共有／保存・読み込み）。PC は「曲を探す」が無いので
  共有だけ 1 行いっぱい。欄の幅が 20rem を切る狭い端末だけ 1 列（2026-09-16）
