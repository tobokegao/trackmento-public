...
- **つまみ（余白・マスの間隔・色の指定）は掴まないと動かない**（`thumbOnly`）。`input[type="range"]` は
  ...
  - **つまみの上を押しただけでも値はその位置へ飛ぶ**（つまみの幅のぶんずれる）。溝を塞いだだけでは
    連打すると少しずつ動いてしまう（2026-09-15 に報告）。`pointerdown` で値を覚え、
    **3px も動かさずに離したら元の値に戻す**（`input` と `change` を投げ直す）。
    確かめ方: `node promo/test_slider.mjs`（マウス）と `node promo/test_slider_touch.mjs`（指）
