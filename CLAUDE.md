...
- **パレット（背景色の 8 色 ＝ 画面の配色）**（2026-09-15、2026-09-16 に 6 → 8 色）。背景色の見出しの
  ...
  - **地と文字を組に入れた影響は画面全体に出る**。`--color-paper` / `--color-ink` は画面の地と文字なので、
    自作の組で近い 2 色を選ぶと**文字が読めなくなる**。`applyPalette` が WCAG の明るさの比を測り、
    **`PAL_MIN_RATIO`（7:1）に満たない組は地と文字だけ既定のまま**にして、一覧にその旨を出す
    （黙って無視しない）。組み込みは リソ 16.6:1、ポップ 14.4:1
  - **中間色（`paper-2` / `paper-3` / `rule` / `rule-2` / `ink-2` / `muted`）は地と文字から作る**（`PALETTE_MIX`）。
    混ぜる割合は、もとからのクリームと黒で今の値とほぼ同じになる値を実測して選んである
    （`paper-2` 6%・`paper-3` 12%・`rule-2` 21%・`rule` 30%・`ink-2` 9%・`muted` 30%）。
    これが無いと、組を替えたとき地だけ変わって枠線や添え字がもとの色のまま残り、ちぐはぐになる
