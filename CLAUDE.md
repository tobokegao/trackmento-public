...
- `/image-proxy` は一度取った画像を R2（`imgcache/`）にも置き、**二度目以降は本体を返さず 302 で R2 へ送る**。画像 1 枚 77KB に対して 302 の応答は数百バイトなので、Render の転送量がほぼ無くなる。`IMAGE_TO_R2=0` で従来どおり本体を返す
  - **一覧の `<img>` は `crossOrigin="anonymous"` で読む**（2026-09-15）。付けずに読むと、その画像は
    「CORS 無し」としてブラウザのキャッシュに載り、**共有画像を描くときの `fetch`（CORS 必須）が
    そのキャッシュに当たって即座に失敗する**。256 マスで実測したところ R2 への読み取りが 510 件すべて失敗し、
    `direct=1`（本体を Render 経由で取り直す）に落ちていた。付けたあとは失敗 0 件・`direct=1` 0 件、
    描画は 9 秒 → 3 秒。**利用者の出力で 150/256 のマスが灰色になっていた原因**
