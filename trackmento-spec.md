...
**2026-09-09 時点の初期仕様の記録**。当時の設計の意図を残すためのもので、今の仕様とは食い違う
（Last.fm は廃止、`/bandcamp` は `/from-url` に、検索元に VocaDB が増えた、など）。
**今の仕様は `CLAUDE.md` と `docs/` が正**。経緯は `docs/history.md`。
...
参考にした既存のサイトがある（アルバム／曲を検索してグリッドに配置 → 画像保存。アートワークは iTunes Search API のみ）。
※ 既存のツールは名前を挙げない方針（2026-09-20）

### 既存ツールでは足りない点
- 曲単位のグリッド作成サイト: 曲単位で選べるが iTunes にない音源（Bandcamp限定リリース等）が拾えない
- Last.fm と Discogs を使うチャート作成ツール: アルバム単位
- Last.fm の再生履歴から自動で作るツール: 手動選択ではない
...
- タイトル表示 ON/OFF、曲名・アーティスト名のサイドバー表示 ON/OFF、背景色、余白
  - 曲名リスト（サイドバー）は 16:9・設定なしでは右、1:1 / 4:5 / 9:16 ではグリッドの下。下のときの列数は 9:16 は 12 曲まで 1 列、1:1 / 4:5 は 6 曲以上で 2 列（正方形で右に置くと内容が横長になり上下の余白ばかり広がるため）
  - タイトルはグリッドの上に置く。ただし右サイドバー（横長・設定なしの比率でリスト表示）のときはサイドバーの上（リストの前）に置く。グリッドの上だと内容が縦長になり 16:9 で左右の余白ばかり広がるため。サイドバー幅はタイトル幅も考慮
  - 余白は「最小値」。比率合わせで余りが出る辺は自動的に広がり、その場合もう一方の辺の余白も内容の短辺の 4%（3×3 で約 73px）以上にして、上下（縦長なら左右）だけ極端に狭くならないようにする。`free` は指定値どおり
...
{ "app": "trackmento", "version": 1, "name": "default", "savedAt": "ISO8601",
  ...
  "options": { "ratio": "1:1|4:5|16:9|9:16|free", "showTitle": true, "sidebar": true,
