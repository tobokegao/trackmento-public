...
4 か所は互いに独立しているので、量が多いときは**1 か所ずつサブエージェントに渡してよい**。
戻ってきた食い違いは、**該当行を自分で開いて確かめてから**直す（取り違え・古い行番号のまま、がよくある）。
...
いちばん先に**利用者に決めてほしいこと**（下の「判断が要るもの」。無ければ「なし」）。
次に、直した箇所を「どこが・何と食い違っていて・何に直したか」の形で並べる。
