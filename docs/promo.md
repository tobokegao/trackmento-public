...
- **X に載せる 1 機能 1 本の短い動画**（2026-09-25）。機能が増えて 1 本の紹介動画に収まらなくなったので、
  ...
  **画面は本物を撮り、演出（カメラ・カーソル・押した合図・繰り返しのつなぎ）は Remotion で足す**（同日、利用者と決めた中間案。
  ...
  - **`sp-share-target` は本物のニコニコ動画のページ**（sm44887188、利用者の指定）を Android の Chrome の名乗り（台本の `ctx.userAgent`）で開く。アプリへの誘導の窓は閉じ、
    第三者の広告（`#Middle320x50Ad` など）と上の帯は撮影のときだけ隠す。ニコニコの「共有」は独自の窓で TRACKMENTO に送れないので、Android の共有シートを描いて重ねる。取り込みは本物
