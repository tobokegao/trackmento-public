...
- **R2 の使用量を数え直すとき**は `PYTHONUTF8=1 .venv/Scripts/python scripts/r2_count.py`（2026-09-21）。
  種類ごとの件数と容量、ボードに渡す引数をそのまま出す。**読むだけで消さない**。
  ...
  - Class A（一覧）を 1,000 件につき 1 回使う。21 万件で 210 回ほど。**毎回の点検では回さない**
  - 2026-09-21 09:40 の実測: 合計 213,740 件・28.50GB（共有 24,649 件 23.2GB、`imgcache/` 133,621 件 4.72GB、
    `uploads/` 1,467 件 0.40GB、`searchcache/` 2,685 件、`fonts/` 1,225 件、`listed/` 807 件）。
    超過 18.5GB ＝ 月 $0.28
