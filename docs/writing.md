...
## Gemma 4 を代わりに使えるか（2026-09-25 に比較。使えない）

3.8 flash の無料枠（1 日 20 回）が切れたときの代わりに、ローカルで動く Gemma 4（Antigravity SDK ＋ LiteRT）を検討した。
先に Gemini API 上の大きい版（`gemma-4-31b-it`・`gemma-4-26b-a4b-it`）で、同じ `STYLE` の添削を比べた。

- 題材: 誤りを 10 か所仕込んだ更新情報の下書き（「しにくく」・「必ず」の言い切り・誤字・「まず最初に」・ら抜き・助詞・
  「することができる」・「エクスポート」・「」の中の画面表記・数値）
- 拾えた数: **3.8 flash 10**、26b-a4b 8、31b 7。Gemma は 2 つとも「まず最初に」の重複と「エクスポート」の言い換えを見落とした。
  31b は「より快適に操作しやすくなります」と新しいぎこちなさを足した
- Gemma は考えた過程が本文に混じる（英語の思考が数十行）。仕込みの無い短い案内文では 2 つとも 2 分でタイムアウトした
- この PC の GPU は Intel Iris Xe（内蔵）。ローカルで動くのは E2B / E4B 級で、上の 31b・26b よりさらに小さい

→ 代わりにしない。枠切れのときは今までどおり下書きを利用者に渡す（メモリ `proofread-fallback-manual`）。
