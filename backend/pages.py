...
CHANGES: list[tuple[str, str, str]] = [
    ("2026-09-26", "書き出しの画像で、下や右の端の空いた段を詰めて組むようにしました（曲の入っていない段のぶん下が空いたり、トラックリストの行の間が広がりすぎたりしていました）。背景が画像やグラデーションのときはトラックリストの後ろに地を敷いて読みやすくし、既定の模様も薄くしました。",
     "Exported images now leave out empty rows and columns at the bottom and right edges (they used to leave a gap below and spread the track list too far apart). With an image or gradient background, the track list gets a backdrop so it is easier to read, and the default pattern is fainter."),
