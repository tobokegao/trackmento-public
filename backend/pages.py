...
"""
    return f"""
<div class="catch">
<p class="catch-main"><span class="ph">「あの頃を思い出す」音MADも、</span><span class="ph">いま心を突き動かすこの曲も。</span><br>
<span class="ph">それでは一緒に、</span><span class="ph">並べてみましょう。</span></p>
<p class="catch-sub"><span class="ph">トラックのメメント</span><span class="ph">「TRACKMENTO」が、</span><span class="ph">あなたのグリッドを、</span><span class="ph">あなただけのセーブポイントにします。</span></p>
</div>
...
<li>サムネイルの色や絵柄でそろえる（「ジャケットが青い曲」など）</li>
...
<li><b>URLから直接追加する：</b>Bandcamp、SoundCloud、YouTube、ニコニコ動画、bilibili、Spotify、Apple Musicのリンク貼り付けに対応しています（Spotifyはトラック名とサムネイルのみのため、アーティスト名はiTunesで同じトラック名のものから補います。見つからないときは手入力してください。bilibiliは、otoDBかVocaDBに登録のある動画だけ取り込めます）。サムネイルがないトラックは、画像URLの指定か「端末から画像を選択」で補えます。</li>
...
CHANGES: list[tuple[str, str, str]] = [
    ("2026-09-27", "「サーバー代のおねがい」の欄の PayPal へのリンクを、昔のソフトの広告のようなバナーに替えました。",
     "In the “A word about server bills” box, the PayPal link is now a banner in the style of old software ads."),
    ...
    ("2026-09-20", "「大きく見る」で、行が少なく列が多い並びのマスを大きく表示するようにしました。",
     "“Enlarge” now shows bigger cells for grids with few rows and many columns."),
    ("2026-09-19", "お問い合わせフォームを設けました。ご質問・不具合のご報告・ご要望・共有データの削除依頼は、「運営者・お問い合わせ」ページのフォームからお送りいただけます。",
     "Added a contact form. You can send questions, bug reports, requests and requests to delete shared data from the form on the “About & contact” page."),
    ...
    ("2026-09-19", "共有のアップロードが終わる直前に、回線が遅い環境で途中で打ち切られることがあったため、待ち時間を延ばしました。",
     "On slow connections, sharing could be cut off just before the upload finished. It now waits longer."),
    ...
    ("2026-09-19", "ニコニコ動画で投稿者名が取得できない曲（退会・非公開・転載動画など）について、VocaDBのデータから作者名を自動で補うようにしました。",
    ...
    ("2026-09-19", "VocaDBの検索処理を改善し、アクセス集中時の時間切れ（タイムアウト）が起きづらくなるようにしました。また、一度検索された曲は一時保存（キャッシュ）により次回以降すばやく表示されるようにしました。",
...
<h2>確認されている不具合</h2>
<ul>
<li><b>古いニコニコ動画の画質：</b>配信元の仕様により低解像度の画像しか存在しないため、サムネイルが粗く表示される場合があります。</li>
<li><b>削除済み動画のサムネイル：</b>otoDBに登録のない削除済み動画は、サムネイル等の情報を復元できません。</li>
...
def _howto(lang: str, days: int) -> str:
    ...
    try:
        ...
        url = {i: st.public_url(k) for i, k in keys.items()}
    except Exception:
        url = {}
    en = lang == "en"
    ...
    for ja_h, en_h, qs in howto.SECTIONS:
        ...
        for vid, jq, ja, eq, ea in qs:
            if not url.get(vid):
                continue
            q, a = (eq, ea) if en else (jq, ja)
            a = a.replace("{days}", str(days))
            items.append(f'<details class="clip" id="{vid}"><summary>{html.escape(q)}</summary><div class="clip-body">'
                         f'<p>{html.escape(a)}</p>'
                         f'<video src="{html.escape(url[vid])}" width="1280" height="720" preload="none" controls muted loop playsinline></video>'
                         f'</div></details>')
        if items:
            out.append(f'<h2>{html.escape(en_h if en else ja_h)}</h2>\n<div class="clips">' + "\n".join(items) + "</div>")
    if len(out) == 1:
        out.append("<p>Videos are being prepared.</p>" if en else "<p>動画は準備中です。</p>")
    return "\n".join(out)
