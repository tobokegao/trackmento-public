...
// 1–3 イントロ
// 4–9   **新しくなったところ**（④翻訳 / ⑥重たくない / ⑤見た目 / 細部 / ③256 マス 2 小節）
// 10–12 トップ画面の説明
// 13–17 入れ方 / 18 複数 URL / 19–20 ①プレイリスト / 21–22 ②復活
// 23–26 並べる / 27 ほぼ完成 / 28–29 出力設定と解像度 / 30–33 色と共有
// 34 タイムラプス / 35–36 できあがり / 37–39 エンドカード
//
// 新機能を頭にまとめたのは、v1 を見た人に「何が変わったか」を先に伝えるため。
// カウントダウン（3,2,1,GO）は v2 では使わない
export const INTRO_END = bar(4);            // イントロは 3 小節
export const FLOW_BEAT = bar(13);           // 操作の流れはここから
export const TIMELAPSE_BEAT = bar(34);
export const SHOWCASE_BEAT = bar(35);       // できあがり（2 小節）
export const BANDWIDTH_BEAT = bar(5);       // ⑥「重たくなくなりました」（録画なしのシーン）
...
export const SHOTS: Shot[] = [
  // ---- 4〜9 小節目: v1 から新しくなったところ（先にここを見せる） ----
  { beat: bar(4), len: 4, ev: "lang:switched", off: -1.2, rec: "feat", jp: "そういえば翻訳ボタンが出来ました", en: "There is a language switch now", zoom: { x: 0.97, y: 0.02, s: 2.1 }, zoomPc: { x: 1, y: 0, s: 2.0 } },
  // 5 小節目は ⑥「重たくなくなりました」（録画ではないので SHOTS には無い。Scenes.tsx が別に描く）
  { beat: bar(6), len: 4, ev: "look:scroll", off: -0.6, rec: "feat", speed: 1.2, ab: "v1-look", jp: "見た目はもっとダサく", en: "Now even uglier", zoom: { x: 0.5, y: 0.78, s: 1.2 }, zoomPc: { x: 0.3, y: 0.5, s: 1.35 } },
  { beat: bar(7), len: 4, ev: "look:options", off: -0.4, rec: "feat", speed: 0.9, jp: "全体の細かい部分も整えました", en: "Lots of small touches", zoomPc: { x: 1, y: 0.5, s: 1.4 } },
  { beat: bar(8), len: 4, ev: "pl:filled", off: -0.6, rec: "feat", jp: "マスは最大 256", en: "Up to 256 cells" },
  { beat: bar(9), len: 4, ev: "cells:wide", off: -1.2, rec: "feat", speed: 1.1, jp: "縦長も横長も自由", en: "Any shape you like", zoom: { x: 0.5, y: 0.86, s: 1.5 }, zoomPc: { x: 1, y: 0.2, s: 1.8 } },

  // ---- 10〜12 小節目: トップ画面の説明 ----
  { beat: bar(10), len: 2, ev: "start", off: -0.9, speed: 0.5, jp: "トップ画面はこれだけ", en: "This is the whole app" },
  { beat: bar(10) + 2, len: 2, ev: "start", off: -0.9, speed: 0.5, jp: "上にタイトル", en: "Title on top", hl: { x: 0.046, y: 0.13, w: 0.909, h: 0.063 }, zoomPc: { x: 0.6, y: 0.2, s: 1.5 } },
  { beat: bar(11), len: 2, ev: "start", off: -0.9, speed: 0.5, jp: "真ん中に 3×3 のマス", en: "Nine cells in the middle", hl: { x: 0.049, y: 0.191, w: 0.872, h: 0.495 }, zoomPc: { x: 0.6, y: 0.45, s: 1.3 } },
  { beat: bar(11) + 2, len: 2, ev: "start", off: -0.9, speed: 0.5, jp: "下に共有と検索、データ保存のボタン", en: "Share, search and save below", hl: { x: 0.046, y: 0.726, w: 0.909, h: 0.17 }, zoomPc: { x: 0.6, y: 0.85, s: 1.5 } },
  { beat: bar(12), len: 4, ev: "look:options", off: -0.4, rec: "feat", speed: 0.9, jp: "出力の設定もここに", en: "Output settings too", zoomPc: { x: 1, y: 0.5, s: 1.4 } },

  // ---- 13〜17 小節目: 入れ方 ----
  { beat: bar(13), len: 4, ev: "title-focus", off: -0.2, jp: "まずはタイトル", en: "Start with a title" },
  { beat: bar(14), len: 4, ev: "cell-tap", off: -0.5, jp: "枠をタップ", en: "Tap a cell", zoomPc: { x: 0.52, y: 0.36, s: 1.8 } },
  { beat: bar(15), len: 4, ev: "search:chikamichi", off: -1.6, speed: 2, jp: "曲を探す", en: "Find tracks", zoomPc: { x: 0, y: 0.3, s: 1.7 } },
  { beat: bar(16), len: 4, ev: "src:musicbrainz", off: -0.4, jp: "検索ソースは 3 種類", en: "Three search sources", zoomPc: { x: 0, y: 0.43, s: 1.9 } },
  { beat: bar(17), len: 4, ev: "url:talk", off: -1.6, speed: 1.4, jp: "URL 検索も対応", en: "Or paste a URL", zoomPc: { x: 0, y: 0.74, s: 1.7 } },
  // 18 小節目: 複数 URL
  { beat: bar(18), len: 4, ev: "multi:got", off: -1.8, rec: "feat", speed: 1.6, jp: "改行で区切って一気に挿入", en: "One per line, all at once" },
  // 19〜20 小節目: ① プレイリスト
  { beat: bar(19), len: 4, ev: "pl:paste", off: -1.8, rec: "feat", speed: 1.6, jp: "プレイリストの URL 内の曲も\nまとめて追加できる", en: "A whole playlist in one paste" },
  { beat: bar(20), len: 4, ev: "pl:fill", off: -1.2, rec: "feat", jp: "最大 500 曲がまとめて入る", en: "Up to 500 tracks at once" },
  // 21〜22 小節目: ② 消えた動画の復活
  { beat: bar(21), len: 4, ev: "revive:paste", off: -1.6, rec: "feat", speed: 1.5, jp: "消えた動画も", en: "Even deleted videos" },
  { beat: bar(22), len: 4, ev: "add:revive", off: -0.8, rec: "feat", jp: "otoDB からよみがえる", en: "come back from otoDB" },
  // ---- 23〜27 小節目: 埋めて並べる ----
  { beat: bar(23), len: 4, ev: "manual:mitsuami", off: -0.6, speed: 1.2, jp: "手入力も可能", en: "Or add your own", zoomPc: { x: 0, y: 1, s: 1.7 } },
  { beat: bar(24), len: 4, ev: "add:ilovelove", off: -0.4, jp: "枠が全部埋まったら", en: "All nine in", zoomPc: { x: 0.6, y: 0.45, s: 1.5 } },
  { beat: bar(25), len: 4, ev: "select:1", off: -0.2, speed: 1.2, jp: "タップで入れ替え", en: "Tap two cells to swap", zoomPc: { x: 0.6, y: 0.45, s: 1.6 } },
  { beat: bar(26), len: 4, ev: "reorder-done", off: -0.3, jp: "並べ終わったら", en: "Once you're done", zoomPc: { x: 0.6, y: 0.45, s: 1.4 } },
  { beat: bar(27), len: 4, ev: "reorder-done", off: 0.6, still: true, jp: "ほぼ完成です", en: "Almost there", zoom: { x: 0.5, y: 0.45, s: 1.35 }, zoomPc: { x: 0.6, y: 0.45, s: 1.35 } },
  // ---- 28〜33 小節目: 出力と共有 ----
  { beat: bar(28), len: 4, ev: "open-options", off: -0.3, fx: "flashIn", jp: "出力方法を設定", en: "Output settings", zoomPc: { x: 1, y: 0.42, s: 1.5 } },
  { beat: bar(29), len: 4, ev: "ratio:16:9", off: -0.3, speed: 1.1, jp: "解像度は 5 種類", en: "Five aspect ratios", zoomPc: { x: 1, y: 0.3, s: 1.9 } },
